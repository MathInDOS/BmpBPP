...
/**
Edited by MathInDOS under BSD (revised/modified)
**/
...
{ 
 ...
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Warning: Attempted to access color table for a BMP object" << endl
        << "                 that lacks a color table. Ignoring request." << endl;
  }
  return Output;
 }
 if( !Colors )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Warning: Requested a color, but the color table" << endl
        << "                 is not defined. Ignoring request." << endl;
  }
  return Output; 
 }
 if( ColorNumber >= TellNumberOfColors() )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Warning: Requested color number " 
        << ColorNumber << " is outside the allowed" << endl
        << "                 range [0," << TellNumberOfColors()-1 
...
RGBApixel* BMP::operator()(int i, int j)
{
 ...
 if( Warn && EasyBMPwarnings )
 {
  cout << "BmpBPP Warning: Attempted to access non-existent pixel;" << endl
       << "                 Truncating request to fit in the range [0,"
       << Width-1 << "] x [0," << Height-1 << "]." << endl;
 }	
 return &(Pixels[i][j]);
...
bool BMP::SetBitDepth( int NewDepth )
{
     ...
     NewDepth != 24 && NewDepth != 32 )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Warning: User attempted to set unsupported bit depth " 
        << NewDepth << "." << endl
...
bool BMP::SetSize(int NewWidth , int NewHeight )
{
 ...
 if( NewWidth <= 0 || NewHeight <= 0 )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Warning: User attempted to set a non-positive width or height." << endl
...
bool BMP::WriteToFile( const char* FileName )
{
 ...
 if( !EasyBMPcheckDataSize() )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Error: Data types are wrong size!" << endl
 ...
 if( fp == NULL )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Error: Cannot open file " 
 ...
 {  
  ...
  while( j > -1 )
  {
   ...
   if( !Success )
   {
    if( EasyBMPwarnings )
    {
     cout << "BmpBPP Error: Could not write proper amount of data." << endl;
	}
    j = -1; 
   }
   j--; 
...
{ 
 ...
 if( bmih.biCompression == 1 || bmih.biCompression == 2 )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Error: " << FileName << " is (RLE) compressed." << endl
 ...
 if( bmih.biCompression > 3 )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Error: " << FileName << " is in an unsupported format." 
        << endl
        << "               (bmih.biCompression = " 
     ...
     && TempBitDepth != 24 && TempBitDepth != 32 )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Error: " << FileName << " has unrecognized bit depth." << endl;
  }
  SetSize(1,1);
  SetBitDepth(1);
  fclose(fp);
 ...
 if( BitDepth < 16 )
 {
  ...
  if( NumberOfColorsToRead < TellNumberOfColors() )
  {
   if( EasyBMPwarnings )
   {
    cout << "BmpBPP Warning: file " << FileName << " has an underspecified" << endl
 ...
 if( BytesToSkip > 0 && BitDepth != 16 )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Warning: Extra meta data detected in file " << FileName << endl
 ...
 if( BitDepth != 16 )
 {
  ...
  while( j > -1 )
  {
   ...
   if( BytesRead < BufferSize )
   {
    ...
    if( EasyBMPwarnings )
    {
     cout << "BmpBPP Error: Could not read proper amount of data." << endl;
	}
   }
   else
   {
    ...
    if( !Success )
    {
     if( EasyBMPwarnings )
     {
      cout << "BmpBPP Error: Could not read enough pixel data!" << endl;
	 }
	 j = -1;
    }
   }   
 ...
 if( BitDepth == 16 )
 {
  ...
  if( BytesToSkip > 0 )
  {
   if( EasyBMPwarnings )
   {
    cout << "BmpBPP Warning: Extra meta data detected in file " 
         << FileName << endl
         << "                 Data will be skipped." << endl;
   }
   ebmpBYTE* TempSkipBYTE;
...
bool BMP::CreateStandardColorTable( void )
{
 ...
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Warning: Attempted to create color table at a bit" << endl
...
BMIH GetBMIH( const char* szFileNameIn )
{
 ...
 if( !fp  )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Error: Cannot initialize from file " 
        << szFileNameIn << "." << endl
        << "               File cannot be opened or does not exist." 
	    << endl;
  }
...
bool EasyBMPcheckDataSize( void )
{
 ...
 if( sizeof( ebmpBYTE ) != 1 )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Error: ebmpBYTE has the wrong size (" 
 ...
 if( sizeof( ebmpWORD ) != 2 )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Error: ebmpWORD has the wrong size (" 
 ...
 if( sizeof( ebmpDWORD ) != 4 )
 {
  if( EasyBMPwarnings )
  {
   cout << "BmpBPP Error: ebmpDWORD has the wrong size (" 
...
bool Rescale( BMP& InputImage , char mode, int NewDimension )
{
     ...
     CapMode != 'F' )
 {
  if( EasyBMPwarnings )
  {
   ...
   sprintf( ErrorMessage, "BmpBPP Error: Unknown rescale mode %c requested\n" , mode );
   cout << ErrorMessage; 
  }
  return false;
 }
